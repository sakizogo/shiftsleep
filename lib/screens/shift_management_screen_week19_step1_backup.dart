...
class ShiftManagementScreenState extends State<ShiftManagementScreen> {
  ...
  void initState() {
    ...
    loadShifts();
    loadCalendarEvents();
    _loadPatterns();  // ========== Week 19 Step 2 追加：DB パターンを読み込む ==========
  }

  // ========== Week 14 追加：DB から patterns を読み込む（Week 19 Step 2 修正） ==========
  Future<void> _loadPatterns() async {
    try {
      final dbPatterns = await _shiftRepository.getAllPatterns();
      setState(() {
        // ========== Week 19 Step 2 修正：デフォルト + DB パターンをマージ ==========
        _patterns = [
          _defaultDayOffPattern,
          ...dbPatterns.where((p) => p.id != 'default_dayoff'),
        ];
        // ======================================================================
      });
      print('[ShiftManagementScreen] Loaded ${dbPatterns.length} patterns from DB');
      print('[ShiftManagementScreen] Total patterns: ${_patterns.length}');
    } catch (e) {
